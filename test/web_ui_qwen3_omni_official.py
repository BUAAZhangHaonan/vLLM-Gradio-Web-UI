    ...
    if args.use_transformers:
        ...
        if args.flash_attn2:
            ...
            model = Qwen3OmniMoeForConditionalGeneration.from_pretrained(
                args.checkpoint_path, device_map="auto", dtype='auto')
    else:
...
def _launch_demo(args, model, processor):
    ...
    def format_history(history: list, system_prompt: str):
        ...
        for item in history:
            ...
            if isinstance(content, str):
                ...
                for file_path in content:
                    ...
                    media_type = "image" if mime_type.startswith("image") else "video" if mime_type.startswith(
                        "video") else "audio" if mime_type.startswith("audio") else None
                    if media_type == "video":
                        file_path = to_mp4(file_path)
                    if media_type:
                        current_user_content.append(
                            {"type": media_type, media_type: file_path})
                    else:
                        current_user_content.append(
                            {"type": "text", "text": file_path})
        if current_user_content:
            media_items = [
                item for item in current_user_content if item["type"] != "text"]
            text_items = [
                item for item in current_user_content if item["type"] == "text"]
            messages.append(
                {"role": "user", "content": media_items + text_items})
        ...
        if indices_to_delete:
            final_messages = [msg for i, msg in enumerate(
                messages) if i not in indices_to_delete]
    ...
    def predict(messages, voice_choice=DEFAULT_VOICE, temperature=0.7, top_p=0.8, top_k=20):
        ...
        if use_transformers:
            ...
            response = processor.batch_decode(
                text_ids.sequences[:, inputs["input_ids"].shape[1]:], skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
            yield {"type": "text", "data": response}
            if audio is not None:
                audio = np.array(
                    audio.reshape(-1).float().detach().cpu().numpy() * 32767).astype(np.int16)
                wav_io = io.BytesIO()
            ...
            sampling_params = SamplingParams(
                temperature=temperature, top_p=top_p, top_k=top_k, max_tokens=16384)
            text = processor.apply_chat_template(
    ...
    def chat_predict(text, audio, image, video, history, system_prompt, voice_choice, temperature, top_p, top_k):
        ...
        if video:
            user_content.append((video,))
        if user_content:
            flat_content = tuple(
                item for sublist in user_content for item in sublist)
        ...
        with gr.Row(equal_height=False):
            with gr.Column(scale=1):
                ...
                system_prompt_textbox = gr.Textbox(
                    label="System Prompt", value=default_system_prompt, lines=4, max_lines=8)
                voice_choice = gr.Dropdown(
                    label="Voice Choice", choices=VOICE_LIST, value=DEFAULT_VOICE, visible=generate_audio)
                temperature = gr.Slider(
                    label="Temperature", minimum=0.1, maximum=2.0, value=0.6, step=0.1)
                top_p = gr.Slider(label="Top P", minimum=0.05,
                                  maximum=1.0, value=0.95, step=0.05)
                top_k = gr.Slider(label="Top K", minimum=1,
                ...
                with gr.Tabs():
                    with gr.TabItem("Online"):
                        with gr.Row():
                            with gr.Column(scale=1):
                                ...
                                microphone = gr.Audio(
                                    sources=['microphone'], type="filepath", label="Record Audio (录制音频)")
                                webcam = gr.Video(sources=[
                                                  'webcam', "upload"], label="Record/Upload Video (录制/上传视频)", elem_classes="media-upload")
                                with gr.Row():
                                    submit_btn_online = gr.Button(
                                        "Submit (提交)", variant="primary", scale=2)
                                    stop_btn_online = gr.Button(
                                        "Stop (停止)", visible=False, scale=1)
                            ...
                            with gr.Column(scale=2):
                                ...
                                media_chatbot = gr.Chatbot(label="Chat History (对话历史)", type="messages", height=650,
                                                           layout="panel", bubble_full_width=False, allow_tags=["think"], render=False)
                        ...
                        submit_event_online = submit_btn_online.click(
                            fn=media_predict,
                            inputs=[microphone, webcam, media_chatbot, system_prompt_textbox,
                                    voice_choice, temperature, top_p, top_k],
                            outputs=[microphone, webcam, media_chatbot,
                                     submit_btn_online, stop_btn_online]
                        )
                        stop_btn_online.click(fn=lambda: (gr.update(visible=True), gr.update(visible=False)), outputs=[
                                              submit_btn_online, stop_btn_online], cancels=[submit_event_online], queue=False)
                        clear_btn_online.click(fn=clear_history_online, outputs=[
                                               media_chatbot, microphone, webcam])
                    ...
                    with gr.TabItem("Offline"):
                        ...
                        with gr.Accordion("📎 Click to upload multimodal files (点击上传多模态文件)", open=False):
                            with gr.Row():
                                audio_input = gr.Audio(sources=[
                                                       "upload", 'microphone'], type="filepath", label="Audio", elem_classes="media-upload")
                                image_input = gr.Image(sources=[
                                                       "upload", 'webcam'], type="filepath", label="Image", elem_classes="media-upload")
                                video_input = gr.Video(
                                    sources=["upload", 'webcam'], label="Video", elem_classes="media-upload")

                        with gr.Row():
                            text_input = gr.Textbox(
                                show_label=False, placeholder="Enter text or upload files and press Submit... (输入文本或者上传文件并点击提交)", scale=7)
                            submit_btn_offline = gr.Button(
                                "Submit (提交)", variant="primary", scale=1)
                            stop_btn_offline = gr.Button(
                                "Stop (停止)", visible=False, scale=1)
                            clear_btn_offline = gr.Button(
                                "Clear (清空) ", scale=1)
                        ...
                        submit_event_offline = gr.on(
                            triggers=[submit_btn_offline.click,
                                      text_input.submit],
                            fn=chat_predict,
                            inputs=[text_input, audio_input, image_input, video_input, chatbot,
                                    system_prompt_textbox, voice_choice, temperature, top_p, top_k],
                            outputs=[text_input, audio_input,
                                     image_input, video_input, chatbot]
                        )
                        stop_btn_offline.click(fn=lambda: (gr.update(visible=True), gr.update(visible=False)), outputs=[
                                               submit_btn_offline, stop_btn_offline], cancels=[submit_event_offline], queue=False)
                        clear_btn_offline.click(fn=clear_history_offline, outputs=[
                                                chatbot, text_input, audio_input, image_input, video_input])
    ...
    demo.queue(default_concurrency_limit=1 if use_transformers else 100, max_size=100).launch(max_threads=100, ssr_mode=False,
...
def _get_args():
    ...
    parser.add_argument('--server-port', type=int,
                        default=8901, help='Demo server port.')
